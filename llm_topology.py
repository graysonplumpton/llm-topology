...
class LLMTopology:
  ...
  def bert_print_contextualized_pca_data(self, prompt, layer=-1, distance_metric='cosine'):
    """
    Perform PCA analysis on contextualized token embeddings from a prompt
  ...
  def bert_print_multi_layer_contextualized_pca(self, prompt, layers=None, distance_metric='cosine'):
    """
    Print PCA data for multiple layers in one go
    """
    if layers is None:
    ...
    for layer in layers:
        print(f"\n--- Processing Layer {layer} ---")
        layer_data = self.bert_print_contextualized_pca_data(prompt, layer, distance_metric)
        all_layer_data[f'layer_{layer}'] = layer_data
  ...
  def print_combined_contextualized_pca_data(self, prompts, layer=-1, distance_metric='cosine'):
    """
    Perform PCA analysis on contextualized token embeddings from multiple prompts combined
    
    Args:
        prompts: List of string prompts to analyze together (e.g., ["The bank by the river", "I went to the bank"])
        layer: Which layer to extract embeddings from (-1 = last layer)
        distance_metric: 'euclidean' or 'cosine' - distance metric for PCA preprocessing
    """
    # Ensure prompts is a list
    ...
    # Collect all embeddings and tokens across all prompts
    all_embeddings = []
    all_tokens = []
    all_prompt_indices = []
    ...
    for prompt_idx, prompt in enumerate(prompts):
        ...
        all_embeddings.append(embeddings_matrix)
        # Prefix tokens with prompt number for identification
        all_tokens.extend([f"P{prompt_idx}:{str(token).strip()}" for token in tokens])
        all_prompt_indices.extend([prompt_idx] * len(tokens))
    ...
    # Apply distance metric transformation if cosine
    if distance_metric.lower() == 'cosine':
        # For cosine distance, normalize the vectors
        from sklearn.preprocessing import normalize
        combined_embeddings = normalize(combined_embeddings, norm='l2')
        distance_name = "Cosine Distance (normalized)"
    elif distance_metric.lower() == 'euclidean':
    ...
    # Print summary first
    print(f"\n=== Combined Contextualized PCA Analysis Summary ===")
    print(f"Prompts: {prompts}")
    print(f"Combined tokens: {len(all_tokens)}")
    print(f"Layer: {layer}")
    print(f"Distance metric: {distance_name}")
  ...
  def print_combined_multi_layer_contextualized_pca(self, prompts, layers=None, distance_metric='cosine'):
    """
    Print combined PCA data for multiple layers - all prompts overlaid in same PCA space per layer
    """
    if layers is None:
        # Default to a good spread of layers
        num_layers = self.model.config.num_hidden_layers
    ...
    print(f"\n=== Combined Multi-Layer Contextualized PCA Analysis ===")
    print(f"Prompts: {prompts}")
    print(f"Distance metric: {distance_metric}")
    print(f"Analyzing layers: {layers}")
    ...
    for layer in layers:
        print(f"\n--- Processing Layer {layer} ---")
        layer_data = self.print_combined_contextualized_pca_data(prompts, layer, distance_metric)
        all_layer_data[f'layer_{layer}'] = layer_data
    
    # Print all data in one big JSON block
    print(f"\n{'='*80}")
    print("COPY ALL COMBINED MULTI-LAYER DATA BELOW:")
    print(f"{'='*80}")
    print(json.dumps(all_layer_data, indent=2))
    print(f"{'='*80}")
  ...
  def find_outlier_token(self, prompt, score="cosine", layer=-1):
    """
    Return the token with the highest clustering score among the last 4 tokens.
    
    Args:
        prompt: Input text prompt
        score: Scoring method ("cosine", "entropy", "euclidean")
        layer: Which model layer to use for embeddings (-1 for last layer)
    """
    # Tokenize the prompt
    inputs = self.tokenizer(prompt, return_tensors="pt", 
                           padding=True, truncation=True).to(self.device)
    ...
    with torch.no_grad():
        ...
        if score == "cosine":
            ...
            if outputs.attentions is None:
                return self.find_outlier_token(prompt, score="cosine", layer=layer)
